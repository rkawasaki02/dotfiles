...
リーダーキーは `Space` です。`Space` 押下後に which-key が候補を表示します（`Space+k` でキーマップの全体一覧）。
...
起動時に表示されます。カーソル移動して `Enter`、または各ショートカットキーで直接実行できます。
...
| `f` / `g` | ファイル検索 / 全文検索 |
| `e` / `t` | Oil / ファイルツリー |
| `s` / `l` | Git status / LazyGit |
| `d` | 診断一覧（Trouble） |
| `c` | Neovim設定を開く |
| `u` / `m` | プラグイン更新 / Mason |
| `q` | 終了 |

フッターには起動時間・プラグイン数・カレントディレクトリのgitブランチが表示されます。
